"""FastAPI application factory. IdP-2: POST /login password verify only."""
...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    async def lifespan(app: FastAPI):
        ...
        app.state.login_service = LoginService(session_factory)
        logger.info(
            "rba-idp ready (IdP-2: password verify only; no PDP). seed app=%s user=%s",
